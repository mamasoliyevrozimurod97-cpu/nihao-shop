...
export default function Header() {
  const { lang, setLang, darkMode, setDarkMode, cart, cartOpen, setCartOpen, localProfile } = useAppStore();
  const t = T[lang];
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="relative">
                <button onClick={() => setLangOpen(!langOpen)} className="flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
                  <Globe size={18} />
                </button>
                {langOpen && (
                  <div className="absolute top-12 right-0 w-32 overflow-hidden rounded-2xl bg-white dark:bg-gray-800 shadow-2xl border border-gray-100 dark:border-gray-700 animate-in fade-in slide-in-from-top-2 duration-200">
                    {LANGS.map(l => (
                      <button key={l} onClick={() => { setLang(l); setLangOpen(false); }} className={`w-full px-4 py-3 text-[10px] font-black uppercase tracking-widest text-left transition ${lang === l ? 'bg-red-600 text-white' : 'hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
                        {l}
                      </button>
                    ))}
                  </div>
              ...
              <button onClick={() => setDarkMode(!darkMode)} className="flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-yellow-50 dark:hover:bg-yellow-900/20 transition-colors">
                {darkMode ? <Sun size={18} /> : <Moon size={18} />}
              </button>

              <button onClick={() => setCartOpen(true)} className="group relative flex h-10 items-center gap-2 rounded-full bg-gray-900 dark:bg-white px-4 text-white dark:text-gray-900 hover:scale-105 transition active:scale-95">
              ...
              <button onClick={() => setAuthOpen(true)} className="flex items-center gap-2 rounded-full bg-gray-100 dark:bg-gray-800 p-1 pr-3 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
                ...
                <div className="hidden flex-col items-start lg:flex">
                  <span className="text-[9px] font-black uppercase tracking-[0.2em] opacity-40 leading-none mb-0.5">{localProfile ? 'Profile' : 'Login'}</span>
                  <span className="text-[10px] font-black truncate max-w-[70px]">{localProfile ? localProfile.firstName : t.login}</span>
                </div>
              </button>
            </div>
          </div>
