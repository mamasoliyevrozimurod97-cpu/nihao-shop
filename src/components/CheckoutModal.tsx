...
import { X, CreditCard, ChevronRight, ShieldCheck, Navigation, ShoppingCart } from "lucide-react";
import { useEffect, useState } from "react";
import { supabase } from "@/lib/supabase";
import { Browser } from "@capacitor/browser";

export default function CheckoutModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  const { lang, cart, clearCart, darkMode, deliveryConfig } = useAppStore();
  const t = T[lang];
  ...
  useEffect(() => {
    const fetchRegions = async () => {
      try {
        const { data } = await supabase.from('delivery_regions').select('*').eq('is_active', true).order('name');
  ...
  }, [isOpen, t.viloyatlar, deliveryConfig.farPrice, form.region]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (prof?.is_blocked) {
        alert("Xatolik: Ushbu telefon raqami bloklangan.");
    ...
    <div className="fixed inset-0 z-[120] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className={`relative w-full max-w-lg overflow-hidden rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl ${dm ? 'bg-gray-950 text-gray-100' : 'bg-white text-gray-900'} animate-in fade-in slide-in-from-bottom sm:zoom-in-95 duration-300 flex flex-col max-h-[95vh]`}>
        <div className={`shrink-0 p-6 border-b flex items-center justify-between ${dm ? 'border-gray-800 bg-gray-900' : 'border-gray-50 bg-white'}`}>
          ...
          <button onClick={onClose} className="rounded-full p-2 hover:bg-gray-100 dark:hover:bg-gray-800 transition"><X size={20} /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 pb-32">
          <section className="space-y-4">
             <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                   <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Ism</label>
                   <input required value={profile.firstName} onChange={e => setProfile({...profile, firstName: e.target.value})} className={inputCls} placeholder="Abdulloh" />
                </div>
                <div className="space-y-1.5">
                   <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Familiya</label>
                   <input required value={profile.lastName} onChange={e => setProfile({...profile, lastName: e.target.value})} className={inputCls} placeholder="Karimov" />
                </div>
             </div>
             <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Sharif (Otchistva)</label>
                <input required value={profile.patronymic || ''} onChange={e => setProfile({...profile, patronymic: e.target.value})} className={inputCls} placeholder="Otasi ismi" />
             </div>
             <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Telefon raqam</label>
                <input required type="tel" value={profile.homePhone} onChange={e => setProfile({...profile, homePhone: e.target.value})} className={inputCls} placeholder="+998..." />
             </div>
             <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                   <div className="flex items-center justify-between ml-2">
                      ...
                      <button type="button" onClick={async () => {
                        ...
                        }} className="text-[9px] font-black text-red-600 uppercase flex items-center gap-1"><Navigation size={10} /> Lokatsiya</button>
                   </div>
                   <input required value={profile.homeAddress} onChange={e => setProfile({...profile, homeAddress: e.target.value})} className={inputCls} placeholder="Ko'cha, uy..." />
                </div>
             </div>
          </section>
          ...
          <div className="flex items-center gap-3 rounded-2xl bg-emerald-50 dark:bg-emerald-900/10 p-4 text-emerald-600 dark:text-emerald-400">
             <ShieldCheck size={20} />
             <p className="text-[10px] font-black uppercase tracking-widest leading-tight">Xavfsiz ulanish. Ma'lumotlar saqlanadi.</p>
          </div>
        </div>

        <div className={`shrink-0 p-6 border-t ${dm ? 'border-gray-800 bg-gray-900' : 'border-gray-50 bg-white'}`}>
          <button disabled={isSubmitting} onClick={handleSubmit} className="w-full rounded-2xl bg-gradient-to-r from-red-600 to-red-700 py-5 text-center text-sm font-black uppercase tracking-widest text-white shadow-2xl transition active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2">
            {isSubmitting ? <div className="h-5 w-5 animate-spin rounded-full border-2 border-white border-t-transparent" /> : <>{t.orderNow} <ChevronRight size={18} /></>}
          </button>
        </div>
      </div>
    </div>
